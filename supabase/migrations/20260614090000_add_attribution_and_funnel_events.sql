-- Widen user_events.event_type CHECK for attribution and funnel events.
--
-- Preserve the live CHECK expression dynamically so this remains safe after
-- parallel event additions and no-ops when these events are already present.
...
DECLARE
  current_check text;
  candidate_events text[] := ARRAY['signed_in','auth_failed','home_viewed','native_app_first_open'];
  missing_events text[];
BEGIN
  ...
  IF missing_events IS NULL OR array_length(missing_events, 1) IS NULL THEN
    RAISE NOTICE 'Attribution and funnel events already present; no change.';
    RETURN;
  END IF;
