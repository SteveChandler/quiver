-- Separate detection policy from permission to send; no policies or authority are seeded.
-- Rollback: keep collection disabled and restore the preceding resolver definition.
BEGIN;
SET LOCAL lock_timeout = '2s';
SET LOCAL statement_timeout = '30s';

CREATE TABLE public.swell_watch_evaluation_policies (
  ...
  CHECK (jsonb_typeof(policy_values #> '{partition_matching,maximum_arrival_delta_hours}') = 'number'
    AND (policy_values #>> '{partition_matching,maximum_arrival_delta_hours}')::numeric > 0
    AND jsonb_typeof(policy_values #> '{partition_matching,maximum_period_delta_s}') = 'number'
    AND (policy_values #>> '{partition_matching,maximum_period_delta_s}')::numeric > 0
    AND jsonb_typeof(policy_values #> '{partition_matching,maximum_direction_delta_deg}') = 'number'
    AND (policy_values #>> '{partition_matching,maximum_direction_delta_deg}')::numeric > 0
    AND (policy_values #>> '{partition_matching,maximum_direction_delta_deg}')::numeric <= 180),
  CHECK (policy_values #> '{partition_matching,maximum_arrival_delta_hours}' IS NOT NULL
    AND policy_values #> '{partition_matching,maximum_period_delta_s}' IS NOT NULL
    AND policy_values #> '{partition_matching,maximum_direction_delta_deg}' IS NOT NULL)
);
ALTER TABLE public.swell_watch_evaluation_policies ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON public.swell_watch_evaluation_policies FROM PUBLIC,anon,authenticated,service_role;

CREATE FUNCTION public.guard_swell_watch_evaluation_policy()
RETURNS trigger LANGUAGE plpgsql SET search_path=public,pg_temp AS $$
BEGIN
  IF TG_OP <> 'INSERT' THEN RAISE EXCEPTION 'evaluation policies are append only'; END IF;
  -- Same lock as detection prevents a policy change during identity selection.
  PERFORM pg_advisory_xact_lock(hashtextextended('swell-watch-control',0));
  IF NEW.epoch <> coalesce((SELECT max(epoch) FROM public.swell_watch_evaluation_policies),0)+1 THEN
    RAISE EXCEPTION 'evaluation policy epoch must advance by one';
  END IF;
  RETURN NEW;
END;
$$;
CREATE TRIGGER swell_watch_evaluation_policy_guard BEFORE INSERT OR UPDATE OR DELETE
ON public.swell_watch_evaluation_policies FOR EACH ROW EXECUTE FUNCTION public.guard_swell_watch_evaluation_policy();

CREATE FUNCTION public.swell_watch_get_matching_policy()
RETURNS TABLE(policy_hash text,policy_values jsonb,not_before timestamptz,expires_at timestamptz,
  revoked_at timestamptz,superseded_at timestamptz)
LANGUAGE plpgsql SECURITY DEFINER SET search_path=public,pg_temp AS $$
DECLARE current_policy public.swell_watch_evaluation_policies;
BEGIN
  PERFORM pg_advisory_xact_lock(hashtextextended('swell-watch-control',0));
  SELECT * INTO current_policy FROM public.swell_watch_evaluation_policies ORDER BY epoch DESC LIMIT 1;
  IF FOUND THEN
    IF current_policy.state='active' AND clock_timestamp() >= current_policy.not_before
      AND clock_timestamp() < current_policy.expires_at THEN
      RETURN QUERY SELECT current_policy.policy_hash,current_policy.policy_values,current_policy.not_before,
        current_policy.expires_at,NULL::timestamptz,NULL::timestamptz;
    END IF;
    RETURN;
  END IF;
  -- Preserve existing approved send operation only until an evaluation ledger is introduced.
  -- Revocation/expiry of that ledger must never fall back to push authority.
  RETURN QUERY SELECT a.policy_hash,a.policy_values,a.not_before,a.expires_at,a.revoked_at,a.superseded_at
    FROM public.swell_watch_get_production_authority() a;
END;
$$;
REVOKE ALL ON FUNCTION public.guard_swell_watch_evaluation_policy(),
  public.swell_watch_get_matching_policy() FROM PUBLIC,anon,authenticated,service_role;

CREATE OR REPLACE FUNCTION public.resolve_and_ingest_swell_watch_evaluation(
...
BEGIN
  ...
  END IF;
  PERFORM pg_advisory_xact_lock(hashtextextended('swell-watch-control',0));
  SELECT * INTO v_authority FROM public.swell_watch_get_matching_policy();
