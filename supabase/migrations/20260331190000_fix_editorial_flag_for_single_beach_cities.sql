-- Fix has_editorial flag to work for single-beach cities
--
-- Previously: has_editorial required >= 2 beaches with editorial content,
-- making it structurally impossible for single-beach cities to pass.
-- Now: uses LEAST(2, COUNT(*)) so single-beach cities need 1 quality beach,
-- and 2+ beach cities still need 2. Semantically: "enough beaches have content."
...
RETURNS TABLE (
  ...
  SELECT
    ...
    ) >= LEAST(2, COUNT(*)) as has_editorial,
    bool_or(
...
has_editorial is true when enough beaches have description AND at least one of crowd_tips/wave_tips/best_conditions_prose.
For single-beach cities: 1 quality beach required. For 2+ beach cities: 2 quality beaches required.
