...
ALTER TABLE public.user_xp ADD CONSTRAINT user_xp_level_check CHECK (level >= 1 AND level <= 10);

ALTER TABLE public.xp_events ADD COLUMN IF NOT EXISTS idempotency_key text;
CREATE UNIQUE INDEX IF NOT EXISTS uniq_xp_events_user_idempotency
  ON public.xp_events (user_id, idempotency_key)
  WHERE idempotency_key IS NOT NULL;

DROP POLICY IF EXISTS "Users can update own XP" ON public.user_xp;
DROP POLICY IF EXISTS "System can insert user XP" ON public.user_xp;
...
COMMENT ON FUNCTION public.increment_user_xp(uuid, integer, text, uuid, text)
  IS 'Idempotent XP award (SECURITY DEFINER); sole write path for user_xp.xp_total. Returns (xp_total, level, awarded); awarded=0 on idempotent replay.';;
