-- Skill-aware board-band match score: shape the user's most-used positive-session
-- board class with session-derived capability before falling back to profile skill,
-- board prior, or intermediate.
--
-- This migration only replaces compute_user_match_score_core. It preserves
-- the public wrapper, grants, and existing JSON keys while adding skill_used
-- and skill_source metadata. No schema change or backfill is needed.
-- Production merge auto-applies this migration; branch verification is required
-- before merging to a prod-bound branch.
