...
-- Revert the OM-primary scalar override from get_bulk_current_forecasts.
-- The displayed wave_height now comes from the per-component face-height
-- transformer (lib/utils/wave-height-transformer.ts), not raw deep-water
-- OM Hs. ML observation matching still reads `wave_height_om` directly.
-- Companion to forecast-builder.ts:275 fix in PR #222 / cherry-pick PR #223.
...
COMMENT ON FUNCTION public.get_bulk_current_forecasts IS
  'Returns the current/next forecast wave_height for each beach. wave_height '
  'is the per-beach face-Hs from the transformer (lib/utils/wave-height-transformer.ts). '
