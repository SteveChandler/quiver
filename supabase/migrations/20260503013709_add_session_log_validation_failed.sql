-- Add 'session_log_validation_failed' to user_events.event_type CHECK.
-- iOS launch-day bug sweep (2026-05-02): native session-form abandonment
-- instrumentation. Fires inside session-form.tsx validation-failure branch
-- so we can see WHY users bounce when validateSessionForm rejects, separate
-- from session_log_abandon (close without submit).
--
-- Strictly additive: every event_type from
-- 20260426150904_extend_user_events_for_anon_alert.sql is preserved verbatim.
