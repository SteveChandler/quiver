...
select
  ...
  (
    -- compute span and center inline
    with params as (
      select
  ...
  round(
    ...
    * 1 -- confidence factor placeholder
  )::int as score_0_100
...
comment on view public.v_beach_hourly_scores is 'Per-hour beach suitability scores (0-100) based on wind (vs offshore), tide band, and swell window; units normalized and wrap-around handled.';
