...
-- The nightly batch filters by the 90-day event window before grouping by user.
-- The existing user-first index is useful for single-user recomputes, but not
-- for the NULL target_user_id batch path.
CREATE INDEX IF NOT EXISTS idx_user_events_implicit_batch
  ON public.user_events (created_at, user_id)
  INCLUDE (event_type, beach_id)
...
COMMENT ON INDEX public.idx_user_events_implicit_batch IS
  'Supports the 90-day batch scan used by compute_implicit_preferences.';
...
COMMENT ON FUNCTION public.compute_implicit_preferences(uuid) IS
  'Computes implicit preferences from opted-in user_events with weighted decay. Uses set-based beach aggregates and metadata-first wave inference to keep the nightly batch within the database statement timeout. Pass NULL for batch processing all users, or user_id for single-user update.';
