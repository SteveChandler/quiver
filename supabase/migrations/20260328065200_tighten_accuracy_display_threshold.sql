...
-- Tighten the should_display threshold in get_yesterday_accuracy.
--
-- Previously, accuracy data was hidden only when BOTH conditions were true:
--   - MAE > 0.45m AND relative error > 40%
-- This was too permissive — bad forecasts where only one condition fired still
-- surfaced on the UI.
--
-- Change: switch the inner AND to OR so accuracy is hidden when EITHER
-- condition is true, making the filter stricter and reducing misleading displays.
