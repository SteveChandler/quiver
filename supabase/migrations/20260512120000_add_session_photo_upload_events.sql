-- Add native session photo upload observability events to user_events.event_type CHECK allowlist.
--
-- The native session photo pipeline emits durable rows for upload start,
-- success, and failure so production E2E can prove picker -> outbox ->
-- storage/session_media -> sessions.image_url without relying on device logs.
--
-- Strictly additive: every event_type from the prior CHECK migration
-- (20260510130000_add_push_registration_observability_events.sql) is preserved
-- verbatim.
