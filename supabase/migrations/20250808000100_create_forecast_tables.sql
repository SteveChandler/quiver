...
create table if not exists public.marine_forecasts (
  ...
  source text not null check (source in ('open-meteo','cdip','ndbc')),
...
create table if not exists public.tide_forecasts (
  ...
  tide_phase text,
  source text not null check (source in ('open-meteo','noaa')),
  created_at timestamptz not null default now()
);
...
create table if not exists public.sun_times (
  ...
  source text not null check (source in ('open-meteo')),
  created_at timestamptz not null default now()
);
