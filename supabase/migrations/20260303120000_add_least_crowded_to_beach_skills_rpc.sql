-- Migration: add_least_crowded_to_beach_skills_rpc
-- Purpose: Add has_least_crowded boolean flag to get_cities_with_beach_skills RPC
-- Matches runtime filter in beach-query-actions.ts: crowd_level IN ('light', 'moderate')

BEGIN;

-- Must drop first because return type changed (added has_least_crowded column)
...
-- Re-grant permissions (DROP + CREATE loses them)
