-- Add native push registration observability events to user_events.event_type CHECK allowlist.
--
-- The native push registration flow now emits durable event rows for
-- permission denial, token fetch failure, backend upsert failure, and
-- successful device registration so production analysis can distinguish
-- those paths without relying on Sentry-only evidence.
--
-- Strictly additive: every event_type from the prior CHECK migration
-- (20260503234500_add_home_hero_forecast_viewed_event.sql) is preserved
-- verbatim.
...
ALTER TABLE public.user_events
  ...
  CHECK (event_type = ANY (ARRAY[
    ...
    -- Added 2026-05-10: native push registration observability.
