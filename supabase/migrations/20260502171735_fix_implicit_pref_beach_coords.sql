-- Fix implicit preference RPC to read the real coordinate columns.
--
-- compute_implicit_preferences (introduced in 20260125120003 and last fixed in
-- 20260502120000) joined `public.beaches` and read `b.center_lat` / `b.center_lng`.
-- Those columns do not exist on `beaches` — the table exposes `lat` / `lon`. The
-- prior versions silently produced rows with location_centroid_lat = null and
-- emitted JSONB with null centroid keys. This migration recreates the function
-- with `b.lat` / `b.lon` and renames the internal JSONB keys to match.
--
-- No external contract changes:
--   * user_implicit_preferences columns (location_centroid_lat / _lon) unchanged.
--   * Internal `beach_data` JSONB keys renamed center_lat→lat, center_lng→lon.
--     Nothing outside this function reads those keys.
