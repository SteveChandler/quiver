...
--
-- Given current conditions at a specific beach, this function scores how similar
-- they are to conditions during the user's best past sessions at that same spot.
-- It uses bucket-based matching across wave height, wave period, wind speed,
-- wind direction, and tide height — weighted and normalized — to produce a 0-10
-- similarity score along with contextual reason bullets and a board recommendation.
--
-- Requires at least 5 rated completed sessions in the last 12 months at the spot.
-- SECURITY DEFINER so it can read across sessions and boards despite RLS.
--
-- ROLLBACK INSTRUCTIONS:
--   DROP FUNCTION IF EXISTS public.compute_spot_similarity_score(UUID, UUID, NUMERIC, NUMERIC, NUMERIC, NUMERIC, NUMERIC);
...
BEGIN
  ...
  -- ============================================================
  -- Step 1: Count eligible sessions at this spot
  -- ============================================================
  SELECT COUNT(*)
  ...
  -- ============================================================
  -- Step 2: Early return if insufficient data
  -- ============================================================
  ...
  -- ============================================================
  -- Steps 3-5: Compute per-session similarity, filter >= 0.60,
  --            take top 5, aggregate score + rating + board tip
  --
  -- Single CTE chain — no cursors, no temp tables.
  -- ============================================================
  WITH session_factors AS (
    -- Extract numeric forecast values from JSONB once per session.
    -- Values may be plain numbers ("3.5") or strings with units ("3.5 ft", "14s").
    -- Use substring to extract leading numeric portion safely.
    SELECT
      s.id                                                          AS session_id,
  ...
  bucket_scores AS (
    SELECT
      ...
      -- Wave Height bucket match (weight 0.35)
      -- Buckets: 0-2 (0), 2-4 (1), 4-6 (2), 6-8 (3), 8+ (4)
      CASE
        WHEN p_wave_height IS NULL OR sf.snap_wh IS NULL THEN NULL
        WHEN (
          CASE WHEN p_wave_height < 2 THEN 0 WHEN p_wave_height < 4 THEN 1
               WHEN p_wave_height < 6 THEN 2 WHEN p_wave_height < 8 THEN 3 ELSE 4 END
        ) = (
          CASE WHEN sf.snap_wh < 2 THEN 0 WHEN sf.snap_wh < 4 THEN 1
               WHEN sf.snap_wh < 6 THEN 2 WHEN sf.snap_wh < 8 THEN 3 ELSE 4 END
        ) THEN 1.0
        WHEN ABS(
          (CASE WHEN p_wave_height < 2 THEN 0 WHEN p_wave_height < 4 THEN 1
                WHEN p_wave_height < 6 THEN 2 WHEN p_wave_height < 8 THEN 3 ELSE 4 END)
      ...
      -- Wave Period bucket match (weight 0.25)
      -- Buckets: 0-8 (0), 8-12 (1), 12-16 (2), 16+ (3)
      CASE
        WHEN p_wave_period IS NULL OR sf.snap_wp IS NULL THEN NULL
        WHEN (
          CASE WHEN p_wave_period < 8 THEN 0 WHEN p_wave_period < 12 THEN 1
               WHEN p_wave_period < 16 THEN 2 ELSE 3 END
        ) = (
          CASE WHEN sf.snap_wp < 8 THEN 0 WHEN sf.snap_wp < 12 THEN 1
               WHEN sf.snap_wp < 16 THEN 2 ELSE 3 END
        ) THEN 1.0
        WHEN ABS(
          (CASE WHEN p_wave_period < 8 THEN 0 WHEN p_wave_period < 12 THEN 1
                WHEN p_wave_period < 16 THEN 2 ELSE 3 END)
      ...
      -- Wind Speed bucket match (weight 0.20)
      -- Buckets: 0-5 (0), 5-10 (1), 10-15 (2), 15+ (3)
      CASE
        WHEN p_wind_speed IS NULL OR sf.snap_ws IS NULL THEN NULL
        WHEN (
          CASE WHEN p_wind_speed < 5 THEN 0 WHEN p_wind_speed < 10 THEN 1
               WHEN p_wind_speed < 15 THEN 2 ELSE 3 END
        ) = (
          CASE WHEN sf.snap_ws < 5 THEN 0 WHEN sf.snap_ws < 10 THEN 1
               WHEN sf.snap_ws < 15 THEN 2 ELSE 3 END
        ) THEN 1.0
        WHEN ABS(
          (CASE WHEN p_wind_speed < 5 THEN 0 WHEN p_wind_speed < 10 THEN 1
                WHEN p_wind_speed < 15 THEN 2 ELSE 3 END)
      ...
      -- Wind Direction circular distance (weight 0.10)
      -- <=45 deg = 1.0, <=90 deg = 0.5, else 0.0
      CASE
        WHEN p_wind_direction IS NULL OR sf.snap_wd IS NULL THEN NULL
        WHEN LEAST(
          ABS(p_wind_direction - sf.snap_wd),
          360.0 - ABS(p_wind_direction - sf.snap_wd)
        ) <= 45 THEN 1.0
        WHEN LEAST(
          ABS(p_wind_direction - sf.snap_wd),
          360.0 - ABS(p_wind_direction - sf.snap_wd)
      ...
      -- Tide Height proximity (weight 0.10)
      -- Within 2ft = 1.0, within 4ft = 0.5, else 0.0
      CASE
  ...
  normalized AS (
    SELECT
      ...
      bs.session_rating,
      bs.board_id,
      -- Available weight = sum of weights for non-NULL factors
      COALESCE(CASE WHEN bs.wh_sim IS NOT NULL THEN 0.35 END, 0)
        ...
        + COALESCE(CASE WHEN bs.th_sim IS NOT NULL THEN 0.10 END, 0)
          AS avail_w,
      -- Weighted sum of available factors
      COALESCE(bs.wh_sim * 0.35, 0)
        + COALESCE(bs.wp_sim * 0.25, 0)
        + COALESCE(bs.ws_sim * 0.20, 0)
        + COALESCE(bs.wd_sim * 0.10, 0)
        + COALESCE(bs.th_sim * 0.10, 0)
          AS w_sum
  ...
  -- Filter >= 0.60 similarity, take top 5 by similarity descending
  top_matches AS (
    SELECT
      ws.session_id,
      ws.session_rating,
  ...
  -- Aggregate score and rating from top matches
  agg AS (
    SELECT
  ...
  -- Board frequency among top matches (only sessions with a board)
  board_freq AS (
    SELECT
      tm.board_id,
    ...
    GROUP BY tm.board_id
    ORDER BY board_count DESC
    LIMIT 1
  )

  -- Pull aggregates and board tip in one SELECT
  SELECT
    a.match_count,
    a.avg_sim,
    a.avg_rat,
    CASE
      WHEN bf.board_id IS NOT NULL
        AND a.match_count > 0
  ...
  -- If no sessions meet the 0.60 threshold, return early with better copy
  ...
  -- ============================================================
  -- Step 5: Map average similarity to 0-10 score
  -- ============================================================
  ...
  -- ============================================================
  -- Step 7: Generate reason_bullets (2-3 entries)
  -- ============================================================
  ...
  -- ============================================================
  -- Final: Assemble and return the JSONB result
  -- ============================================================
  RETURN jsonb_build_object(
    'score',          v_score,
...
COMMENT ON FUNCTION public.compute_spot_similarity_score(UUID, UUID, NUMERIC, NUMERIC, NUMERIC, NUMERIC, NUMERIC) IS
  'Computes a 0-10 similarity score for current conditions at a specific beach '
  'relative to a user''s best past sessions there. Uses bucket-based matching across '
  'wave height (0.35), wave period (0.25), wind speed (0.20), wind direction (0.10), '
  'and tide height (0.10). Requires >= 5 rated completed sessions in the last 12 months. '
  'Returns score, label, reason bullets, and optional board recommendation. '
  'SECURITY DEFINER — access restricted to the specified user_id row set.';
