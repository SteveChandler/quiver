-- Fix redundant IS NOT NULL predicates in beach_ml_performance_baseline
--
-- The materialized view used `p.observed_m IS NOT NULL AND p.observed_m > 0`
-- in several places. The IS NOT NULL is redundant when > 0 is present.
-- Also used COUNT(p.observed_m) FILTER (...) which is equivalent to
-- COUNT(*) FILTER (...) given the filter ensures non-null, but is
-- inconsistent with the > 0 convention established in migration
-- 20260222130000_fix_sentinel_filtering_ml_functions.sql.
--
-- This migration recreates the view with consistent predicates.

BEGIN;

-- Drop and recreate (materialized views don't support CREATE OR REPLACE)
...
-- Recreate dependent functions (dropped by CASCADE)
