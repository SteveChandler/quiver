...
BEGIN
    ...
    SELECT id INTO dawn_patrol_id FROM profiles WHERE full_name = 'Dawn Patrol' LIMIT 1;
    SELECT id INTO p_martinez_id FROM profiles WHERE full_name = 'P. Martinez' LIMIT 1;
    SELECT id INTO local_larry_id FROM profiles WHERE full_name = 'Local Larry' LIMIT 1;
    SELECT id INTO tina_c_id FROM profiles WHERE full_name = 'Tina C.' LIMIT 1;
    SELECT id INTO riley_r_id FROM profiles WHERE full_name = 'Riley R.' LIMIT 1;
    ...
    -- ================================================
    -- OCEAN BEACH - Local Larry's Home Turf
    -- ================================================

    IF ocean_beach_id IS NOT NULL AND local_larry_id IS NOT NULL THEN
        ...
        (local_larry_id, ocean_beach_id, 32.7507, -117.254, 'conditions', 'OB Firing Right Now!', 
        ...
        (local_larry_id, ocean_beach_id, 32.7507, -117.254, 'parking', 'OB Parking Update - Newport Ave',
        ...
        (local_larry_id, ocean_beach_id, 32.7507, -117.254, 'crowd', 'Classic OB Local Vibe',
    ...
    IF pacific_beach_id IS NOT NULL THEN
        INSERT INTO intel_posts (user_id, beach_id, latitude, longitude, tag, title, description, surf_conditions, confirmations_count, is_active, expires_at, created_at) VALUES
        (p_martinez_id, pacific_beach_id, 32.7979, -117.255, 'conditions', 'PB Perfect for Photos Right Now',
        ...
        (riley_r_id, pacific_beach_id, 32.7979, -117.255, 'crowd', 'Great Learning Environment Today',
        ...
        (tina_c_id, pacific_beach_id, 32.7979, -117.255, 'parking', 'PB Parking Strategy',
    ...
    IF mission_beach_id IS NOT NULL THEN
        INSERT INTO intel_posts (user_id, beach_id, latitude, longitude, tag, title, description, surf_conditions, confirmations_count, is_active, expires_at, created_at) VALUES
        (riley_r_id, mission_beach_id, 32.7702, -117.2525, 'conditions', 'Perfect Learning Waves!',
        ...
        (p_martinez_id, mission_beach_id, 32.7702, -117.2525, 'other', 'Weekend Beach Scene',
    ...
    IF la_jolla_shores_id IS NOT NULL THEN
        INSERT INTO intel_posts (user_id, beach_id, latitude, longitude, tag, title, description, surf_conditions, confirmations_count, is_active, expires_at, created_at) VALUES
        (dawn_patrol_id, la_jolla_shores_id, 32.8507, -117.2726, 'conditions', 'Dawn Patrol Gold at Shores',
        ...
        (riley_r_id, la_jolla_shores_id, 32.8507, -117.2726, 'access', 'Easiest Beach Access Ever',
        ...
        (p_martinez_id, la_jolla_shores_id, 32.8507, -117.2726, 'other', 'Underwater Photography Paradise',
    ...
    IF windansea_id IS NOT NULL THEN
        ...
        (tina_c_id, windansea_id, 32.8371, -117.2791, 'crowd', 'Local Lineup Dynamics',
    ...
    IF blacks_beach_id IS NOT NULL THEN
        ...
        (dawn_patrol_id, blacks_beach_id, 32.9016, -117.2524, 'hazard', 'Strong Current Advisory - Dawn Patrol',
    ...
    IF sunset_cliffs_id IS NOT NULL THEN
        ...
        (p_martinez_id, sunset_cliffs_id, 32.7351, -117.2519, 'other', 'Golden Hour Photography Magic',
        ...
        (tina_c_id, sunset_cliffs_id, 32.7351, -117.2519, 'hazard', 'Cliff Jump Hazard Warning',
    ...
    IF swamis_id IS NOT NULL THEN
        INSERT INTO intel_posts (user_id, beach_id, latitude, longitude, tag, title, description, surf_conditions, confirmations_count, is_active, expires_at, created_at) VALUES
        (dawn_patrol_id, swamis_id, 33.0362, -117.3032, 'conditions', 'Swamis Dawn Patrol Perfection',
        ...
        (tina_c_id, swamis_id, 33.0362, -117.3032, 'parking', 'Swamis Parking Nightmare',
    ...
    IF mission_beach_id IS NOT NULL THEN
        INSERT INTO intel_posts (user_id, beach_id, latitude, longitude, tag, title, description, surf_conditions, confirmations_count, is_active, expires_at, created_at) VALUES
        (riley_r_id, mission_beach_id, 32.7702, -117.2525, 'conditions', 'Perfect for New Surfers Today',
        ...
        (p_martinez_id, mission_beach_id, 32.7702, -117.2525, 'crowd', 'Weekend Beach Culture Documentation',
    ...
    IF la_jolla_shores_id IS NOT NULL THEN
        INSERT INTO intel_posts (user_id, beach_id, latitude, longitude, tag, title, description, surf_conditions, confirmations_count, is_active, expires_at, created_at) VALUES
        (tina_c_id, la_jolla_shores_id, 32.8507, -117.2726, 'other', 'Travel Review: Perfect Beginner Spot',
        ...
        (local_larry_id, la_jolla_shores_id, 32.8507, -117.2726, 'parking', 'Shores Parking Pro Tips',
    ...
    SELECT DISTINCT
        ...
        CASE (RANDOM() * 8)::INTEGER
            ...
            WHEN 3 THEN dawn_patrol_id
            WHEN 4 THEN p_martinez_id
            WHEN 5 THEN local_larry_id
            WHEN 6 THEN tina_c_id
            ELSE riley_r_id
        END,
    ...
    RAISE NOTICE '🏠 Local Larry: Ocean Beach local knowledge (3 posts)';
    RAISE NOTICE '📸 P. Martinez: Photography insights (4 posts)';
    RAISE NOTICE '🏄‍♀️ Riley R.: Beginner-friendly observations (3 posts)';
    RAISE NOTICE '🌅 Dawn Patrol: Dawn patrol conditions (2 posts)';
    ...
    RAISE NOTICE '🚐 Tina C.: Travel perspectives (4 posts)';
    RAISE NOTICE '';
