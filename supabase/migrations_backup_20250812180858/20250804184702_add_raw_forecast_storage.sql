...
DO $$ BEGIN
  ...
  ELSE
    RAISE NOTICE 'Skipping raw_forecast addition: enhanced_forecasts not found';
  END IF;
END $$;
...
DO $$ BEGIN
  IF to_regclass('public.enhanced_forecasts') IS NOT NULL THEN
    ...
    ON public.enhanced_forecasts USING GIN ((raw_forecast->'data_sources'));
  END IF;
END $$;
...
DO $$ BEGIN
  IF to_regclass('public.enhanced_forecasts') IS NOT NULL THEN
    ...
    ON public.enhanced_forecasts USING GIN ((raw_forecast->'cdip_data'));
  END IF;
END $$;
...
DO $$ BEGIN
  IF to_regclass('public.enhanced_forecasts') IS NOT NULL THEN
    ...
    ON public.enhanced_forecasts USING GIN ((raw_forecast->'quality_scores'));
  END IF;
END $$;
...
BEGIN
    ...
    IF to_regclass('public.enhanced_forecasts') IS NOT NULL AND EXISTS (
        SELECT 1 
...
CREATE OR REPLACE FUNCTION public.validate_raw_forecast_structure(raw_data JSONB)
RETURNS BOOLEAN AS $$
BEGIN
...
BEGIN
    IF to_regclass('public.enhanced_forecasts') IS NOT NULL THEN
        ...
        IF NOT EXISTS (
            ...
            CHECK (public.validate_raw_forecast_structure(raw_forecast));
        END IF;
    END IF;
END $$;

-- Create a function to extract data quality score from raw_forecast
CREATE OR REPLACE FUNCTION public.get_overall_quality_score(raw_data JSONB)
RETURNS INTEGER AS $$
BEGIN
    IF raw_data IS NULL THEN
        RETURN NULL;
...
DO $$ BEGIN
  IF to_regclass('public.enhanced_forecasts') IS NOT NULL THEN
CREATE OR REPLACE VIEW public.enhanced_forecasts_with_quality AS
SELECT 
    ef.*,
...
ON public.enhanced_forecasts (get_overall_quality_score(raw_forecast));

-- Add comments for documentation
COMMENT ON COLUMN public.enhanced_forecasts.raw_forecast IS 'JSONB field storing unprocessed API responses from CDIP, NOAA, and other data sources for debugging and analysis';

COMMENT ON FUNCTION public.validate_raw_forecast_structure(JSONB) IS 'Validates the structure of raw_forecast JSONB data to ensure required fields are present';

COMMENT ON FUNCTION public.get_overall_quality_score(JSONB) IS 'Extracts or calculates overall data quality score from raw_forecast JSONB data';
...
GRANT EXECUTE ON FUNCTION public.validate_raw_forecast_structure(JSONB) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_overall_quality_score(JSONB) TO authenticated;
  END IF;
END $$;