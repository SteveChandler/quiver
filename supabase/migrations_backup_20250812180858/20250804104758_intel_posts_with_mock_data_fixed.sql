...
-- Guard mock inserts for local environments missing auth.users
DO $$
BEGIN
...
BEGIN
    IF to_regclass('auth.users') IS NULL THEN
        -- Local stack without auth schema: skip mock data
        RETURN;
    END IF;

    -- Ensure required test users exist; otherwise skip to avoid FK errors
    IF (
      SELECT COUNT(*) FROM auth.users 
      WHERE id IN (
        '23233d36-97f9-4322-8b36-113c880b841f'::uuid,
        '16b87cb1-34b6-434d-820c-0bc4e0927f5b'::uuid,
        '638edbd2-7fd3-49b5-8129-84456764df4c'::uuid,
        '05f3d22c-a282-4252-bb54-64dcb74a83dd'::uuid,
        '382c284f-e36d-43cd-8e44-0930544db459'::uuid,
        '701beae0-96aa-43d4-a29a-4353ead6ea24'::uuid,
        '31d78eb7-357e-444e-88cd-d728ebf4f1ae'::uuid,
        '3958e9bb-acf8-45a1-ab20-7953ec1cb0e7'::uuid
      )
    ) < 8 THEN
      RAISE NOTICE 'Skipping intel mock data insertion: required users not present in auth.users';
      RETURN;
    END IF;
