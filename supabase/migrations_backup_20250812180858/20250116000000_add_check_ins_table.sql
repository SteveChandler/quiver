...
BEGIN
  IF to_regclass('public.beaches') IS NOT NULL THEN
    ...
    $ct$;
  ELSE
    RAISE NOTICE 'Skipping check_ins creation: required table public.beaches does not exist';
  END IF;
END $$;
...
BEGIN
  IF to_regclass('public.check_ins') IS NOT NULL THEN
    ...
    CREATE INDEX IF NOT EXISTS idx_check_ins_beach_id ON public.check_ins(beach_id);
    CREATE INDEX IF NOT EXISTS idx_check_ins_checked_in_at ON public.check_ins(checked_in_at DESC);
    CREATE INDEX IF NOT EXISTS idx_check_ins_beach_checked_in_at ON public.check_ins(beach_id, checked_in_at DESC);
    CREATE INDEX IF NOT EXISTS idx_check_ins_forecast_accuracy ON public.check_ins(forecast_accuracy_rating);
  END IF;
END $$;
...
BEGIN
  IF to_regclass('public.check_ins') IS NOT NULL THEN
    IF EXISTS (
      ...
      WHERE schemaname = 'public' AND tablename = 'check_ins' AND policyname = 'check_ins_select_public'
    ) THEN
      DROP POLICY "check_ins_select_public" ON public.check_ins;
    END IF;
...
BEGIN
  IF to_regclass('public.check_ins') IS NOT NULL THEN
    IF EXISTS (
      SELECT 1 FROM pg_policies 
      WHERE schemaname = 'public' AND tablename = 'check_ins' AND policyname = 'check_ins_insert_own'
    ) THEN
...
BEGIN
  IF to_regclass('public.check_ins') IS NOT NULL THEN
    IF EXISTS (
      SELECT 1 FROM pg_policies 
      WHERE schemaname = 'public' AND tablename = 'check_ins' AND policyname = 'check_ins_update_own'
    ) THEN
...
BEGIN
  IF to_regclass('public.check_ins') IS NOT NULL THEN
    IF EXISTS (
      SELECT 1 FROM pg_policies 
      WHERE schemaname = 'public' AND tablename = 'check_ins' AND policyname = 'check_ins_delete_own'
    ) THEN
      DROP POLICY "check_ins_delete_own" ON public.check_ins;
    END IF;
...
BEGIN
  IF to_regclass('public.check_ins') IS NOT NULL THEN
    DROP TRIGGER IF EXISTS trigger_check_ins_updated_at ON public.check_ins;
