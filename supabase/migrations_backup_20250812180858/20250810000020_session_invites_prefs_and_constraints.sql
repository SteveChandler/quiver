...
do $$ begin
  if to_regclass('public.session_invitations') is not null then
    ...
    raise notice 'Skipping status check: session_invitations not found';
  end if;
end $$;
...
do $$ begin
  if to_regclass('public.session_invitations') is not null then
    ...
    raise notice 'Skipping idempotency index: session_invitations not found';
  end if;
end $$;
...
begin
  ...
  if to_regclass('public.session_invitations') is not null and not exists (
    select 1 from pg_constraint
