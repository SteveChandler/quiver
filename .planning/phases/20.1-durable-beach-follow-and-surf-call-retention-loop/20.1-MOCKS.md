...
**Date:** 2026-08-24  
**Purpose:** Make the intended cross-surface loop concrete before implementation and prevent the phase from collapsing into generic CTAs, another alert composer, or an invented planning state machine.
...
### 6. Material update → one-tap outcome → truthful receipt

![Native outcome mock](mocks/06-native-outcome.svg)

**Proves:**

- Notifications interrupt only for `Still on`, `Call changed`, `Better nearby`, or `How was it?`.
- A notification opens the exact watched call with prior/current context.
- Post-window feedback requires only a 1–5 answer; beach/window/board/recommendation are prefilled.
- The full session log is optional.
- The receipt says what was actually saved or recalculated and does not claim that one response trained the forecast.

**Allowed receipt examples:**

- `Saved to your Scripps history.`
- `This rating will be available when Quiver compares similar sessions.`
- `Your board-fit preference was updated.` — only if that write actually occurred.

**Disallowed receipt examples:**

- `The forecast is now more accurate.`
- `Quiver learned this beach from one rating.`
- `Your model has been retrained.`
...
Surf-qualified visitor
  ...
  5. Quiver sends only a useful update.
  6. User rates the outcome in one tap.
  7. Quiver records a truthful receipt.
```
...
- The 1–5 outcome control has explicit accessibility labels such as `1, poor` through `5, excellent`.
...
| 06 | Push/deep-link, one-tap rating, offline/retry, receipt | Useful interruption, lightweight outcome, truthful persistence claim. |
...
- No mandatory full session log after a watched call.
