...
export async function fetchForecastData(beach: Beach, apiKey: string) {
  try {
    ...
    const weatherResponse = await fetch(
      `${STORMGLASS_WEATHER_ENDPOINT}?lat=${beach.latitude}&lng=${beach.longitude}&params=${WEATHER_PARAMS.join(",")}`,
      {
        headers: {
    ...
    // Fetch tide data
    const tideResponse = await fetch(`${STORMGLASS_TIDE_ENDPOINT}?lat=${beach.latitude}&lng=${beach.longitude}`, {
      headers: {
        Authorization: apiKey,
      },
    })
    ...
    if (!tideResponse.ok) {
      throw new Error(`Tide API error: ${tideResponse.status} ${tideResponse.statusText}`)
    }
    ...
    const forecasts = weatherData.hours.map((hour: any) => {
      ...
      const weatherCondition = getWeatherCondition(hour.cloudCover[source], hour.precipitation[source])
