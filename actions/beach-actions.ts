...
export async function getAllBeaches() {
  ...
  try {
    const { data, error } = await supabase.from("beaches").select("*").order("name")
...
export async function getNearbyBeaches(latitude: number, longitude: number, radius = 50) {
  const supabase = createServerClient()
...
export async function getFavoriteBeaches(userId: string) {
  ...
  try {
    ...
    const beaches = data.map((item) => item.beaches) as Beach[]
...
export async function removeFavoriteBeach(userId: string, beachId: string) {
  ...
  try {
    const { error } = await supabase.from("favorite_beaches").delete().eq("user_id", userId).eq("beach_id", beachId)
