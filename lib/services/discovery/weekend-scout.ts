...
export interface WeekendScoutDependencies {
  ...
  generateForecast: (userId: string, request: WeekScoutDaysRequest) => Promise<WeekScoutResponse>;
}
...
interface RankedWindow {
  ...
  window: WeekScoutWindowResponse;
...
function defaultDependencies(now: Date): WeekendScoutDependencies {
  ...
  return {
    ...
    generateForecast: (userId, request) => generateWeekScoutForecastForDays(userId, request),
  };
}
...
function rankedWindows(
  ...
  for (const window of forecast.days.flatMap((day) => day.windows)) {
    if (!window.safe || !window.rideable || window.verdict !== 'worth_it') continue;
    const candidate = candidatesById.get(window.beachId);
    if (!candidate) continue;
    const adjustedScore = Math.max(
      0,
      Math.min(100, window.rankingScore + calculateDistancePenalty(candidate.distanceMiles)),
    );
    const ranked = { candidate, window, adjustedScore };
...
export async function buildWeekendScoutRanking(
  ...
  if (!Number.isFinite(capturedAt) || deps.now.getTime() - capturedAt > LOCATION_MAX_AGE_MS) {
