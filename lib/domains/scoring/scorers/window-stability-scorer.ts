...
/**
 * Weights for combining wave and wind stability scores.
 * Wave stability is weighted more heavily as it has greater impact on surf quality.
 */
const STABILITY_WEIGHTS = {
  /** Wave stability contribution to combined score */
  wave: 0.6,
  /** Wind stability contribution to combined score */
  wind: 0.4,
};
...
export const windowStabilityScorer: ScorerPlugin = {
  ...
  score(input: ScorerInput): ScorerResult {
    ...
    const combinedScore = Math.round(
      waveScore * STABILITY_WEIGHTS.wave + windScore * STABILITY_WEIGHTS.wind
    );
