-- DRAFT: owner execution only after backup, deployment/configuration verification, and final approval.
-- This changes the evaluation-only ledger. It neither changes runtime flags nor grants push authority.
-- Binds docs/operations/swell-watch-no-send-producer-config-v2-proposed.json
-- SHA-256: 6ac158d19ffd55505609efae9ce6b634bea9d92342c133333fdc91fe29471403
BEGIN;
...
BEGIN
  ...
  IF v_epoch1.epoch IS DISTINCT FROM 1
    ...
    OR v_epoch1.expires_at <= clock_timestamp() THEN
    RAISE EXCEPTION 'Epoch 1 is not the reviewed, current v1 policy; do not transition';
  END IF;
  ...
  IF v_count = 1 THEN
    ...
    VALUES
      (2, 'active', v_v2_hash, v_v2_values, v_v2_reviewer, v_v2_evidence,
  ...
  IF v_count = 2 THEN
    ...
    IF v_epoch2.epoch = 2
      AND v_epoch2.state = 'active'
      AND v_epoch2.policy_hash = v_v2_hash
      AND v_epoch2.policy_values = v_v2_values
      AND v_epoch2.reviewer = v_v2_reviewer
      AND v_epoch2.evidence_hash = v_v2_evidence
      AND v_epoch2.not_before = v_old_not_before
      AND v_epoch2.expires_at = v_original_expiry
      AND v_epoch2.expires_at > clock_timestamp() THEN
      RETURN;
    END IF;
  END IF;
