...
BEGIN
  ...
  IF (SELECT state FROM public.swell_watch_get_automation_control()) IS DISTINCT FROM 'disabled'
    OR EXISTS(SELECT 1 FROM public.swell_watch_get_production_authority()) THEN
    RAISE EXCEPTION 'Swell Watch send authority must remain disabled';
  END IF;
  ...
  END IF;
    IF existing.epoch <> 1 OR existing.state <> 'active' OR existing.expires_at <= clock_timestamp() THEN
      RAISE EXCEPTION 'Policy is not a live initial install; do not renew implicitly';
    END IF;
    RETURN;
  END IF;
  ...
  VALUES (1,'active','4c9ec372e9dff824039956ef5d2f46e6d3445b9d0d80d6626cac9932de436ecd',expected_values,'Steven Chandler','f8d3549aec17646288a8a00bb174075973f2ab461b7506315c5c5aab5784d977',
    transaction_timestamp(),transaction_timestamp()+interval '45 days');
END;
$operation$;
COMMIT;
