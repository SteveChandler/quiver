-- Revoke epoch 3 first with swell-watch-study-revoke-partition-coverage.sql, then restore these functions.
-- The harmless qualification_rule column and all authority/evidence rows remain.
-- These are the exact pre-migration pg_get_functiondef definitions; grants are preserved.
BEGIN;
...
END $$;
-- guard_swell_watch_study_authority(): pre 0746463f7308dc48acce42a70dfe3c01dc72e0ac97f6540e294d80dd16d808e8; post 8480f4860778224e8ec6ba308e165ad0990fa0cab0b8aafc7d51cdd722e66614.
DO $amend$
DECLARE definition text;
BEGIN
  SELECT pg_get_functiondef('public.guard_swell_watch_study_authority()'::regprocedure) INTO definition;
  IF encode(extensions.digest(definition,'sha256'),'hex')='8480f4860778224e8ec6ba308e165ad0990fa0cab0b8aafc7d51cdd722e66614' THEN RETURN; END IF;
...
-- read_swell_watch_study_health(): pre b2789dfdb0637335290be5883ef57f19e2889cfa071d1ecbadd6ad9b72b30c01; post 5c7183ca088c2ed6f41c8340584ecb8ff015f578368d892b25530816a0ec8c2e.
DO $amend$
DECLARE definition text;
BEGIN
  SELECT pg_get_functiondef('public.read_swell_watch_study_health()'::regprocedure) INTO definition;
  IF encode(extensions.digest(definition,'sha256'),'hex')='5c7183ca088c2ed6f41c8340584ecb8ff015f578368d892b25530816a0ec8c2e' THEN RETURN; END IF;
...
-- record_swell_watch_study_evaluation(uuid,text,jsonb,jsonb): pre e0e0e7f5d09ce8e3d2b668dd4c022ed429ff38d57473ae803ee093620c9f5521; post ead27c82c4e7b7cb3d52ac05b57e7eeb7e5b550f05a07a66de19b725abd77223.
DO $amend$
DECLARE definition text;
BEGIN
  SELECT pg_get_functiondef('public.record_swell_watch_study_evaluation(uuid,text,jsonb,jsonb)'::regprocedure) INTO definition;
  IF encode(extensions.digest(definition,'sha256'),'hex')='ead27c82c4e7b7cb3d52ac05b57e7eeb7e5b550f05a07a66de19b725abd77223' THEN RETURN; END IF;
