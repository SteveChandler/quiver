-- Rollback receipt writer only; retain all evidence and normalization provenance. Only raw direction exactly 360 with a positive period becomes 0.
...
BEGIN
  ...
  IF encode(extensions.digest(definition,'sha256'),'hex')='26d064b92fb0023f079d399ca062833f7d9a882d53d1f1e9ab55ef1a5726f522' THEN RETURN; END IF;
  ...
  FOR patch IN SELECT * FROM (VALUES
    ($old$(v_component->>'directionDeg')::numeric IS DISTINCT FROM (CASE WHEN (v_raw->'hourly'->'swell_wave_direction'->>v_index)::numeric=360 AND (v_component->>'periodS')::numeric>0 THEN 0 ELSE (v_raw->'hourly'->'swell_wave_direction'->>v_index)::numeric END)$old$,$new$(v_component->>'directionDeg')::numeric IS DISTINCT FROM (v_raw->'hourly'->'swell_wave_direction'->>v_index)::numeric$new$),
    ($old$v_component->'rawFieldProvenance' IS DISTINCT FROM ('{"height":"swell_wave_height","period":"swell_wave_period","direction":"swell_wave_direction"}'::jsonb || CASE WHEN (v_raw->'hourly'->'swell_wave_direction'->>v_index)::numeric=360 THEN '{"directionNormalization":{"rule":"north_360_to_0.v1","rawDegrees":360,"canonicalDegrees":0}}'::jsonb ELSE '{}'::jsonb END)$old$,$new$v_component->'rawFieldProvenance' IS DISTINCT FROM '{"height":"swell_wave_height","period":"swell_wave_period","direction":"swell_wave_direction"}'::jsonb$new$),
    ($old$(v_component->>'directionDeg')::numeric IS DISTINCT FROM (CASE WHEN (v_raw->'hourly'->'secondary_swell_wave_direction'->>v_index)::numeric=360 AND (v_component->>'periodS')::numeric>0 THEN 0 ELSE (v_raw->'hourly'->'secondary_swell_wave_direction'->>v_index)::numeric END)$old$,$new$(v_component->>'directionDeg')::numeric IS DISTINCT FROM (v_raw->'hourly'->'secondary_swell_wave_direction'->>v_index)::numeric$new$),
    ($old$v_component->'rawFieldProvenance' IS DISTINCT FROM ('{"height":"secondary_swell_wave_height","period":"secondary_swell_wave_period","direction":"secondary_swell_wave_direction"}'::jsonb || CASE WHEN (v_raw->'hourly'->'secondary_swell_wave_direction'->>v_index)::numeric=360 THEN '{"directionNormalization":{"rule":"north_360_to_0.v1","rawDegrees":360,"canonicalDegrees":0}}'::jsonb ELSE '{}'::jsonb END)$old$,$new$v_component->'rawFieldProvenance' IS DISTINCT FROM '{"height":"secondary_swell_wave_height","period":"secondary_swell_wave_period","direction":"secondary_swell_wave_direction"}'::jsonb$new$)
