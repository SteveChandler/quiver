...
> **For the implementing agent (Codex):** You have zero prior context. Read the design spec `docs/superpowers/specs/2026-06-15-cdip-direction-shadow-guard-design.md` first — it has the physics rationale and the decisions. Then work these tasks in order. Each is TDD: write the failing test, run it red, implement, run it green, commit. Steps use `- [ ]` checkboxes.
