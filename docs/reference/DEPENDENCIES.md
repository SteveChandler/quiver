...
- **Manual registration**: Service worker generation is handled by `next-pwa`, but registration is runtime-controlled in `components/analytics/pwa-and-push-listeners.tsx` (prevents localhost caching/hydration regressions)
