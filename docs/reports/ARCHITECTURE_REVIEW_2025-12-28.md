...
- Repo guidance references a top-level `docs/ARCHITECTURE_REVIEW.md`, but the current structure uses `docs/ARCHITECTURE.md` + `docs/architecture/*` + reports in `docs/reports/`.
...
- Add a lightweight “Architecture review index” in `docs/reports/` (optional) so these system-level reviews remain discoverable.
