...
network request. Quiver has no separate legal department; the founder/operator
owns policy approval. Do not enable the roster until the approved Terms and
Privacy copy has completed its production-publication checkpoint.
...
1. Record founder/operator approval and production-publication proof for the
   exact `/terms` and `/privacy` copy.
