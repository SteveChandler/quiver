...
| spot page | yes - today/tomorrow 48-row surf-call path in `getSpotSurfReportPublic` and `actions/spot/spot-surf-report-actions.ts` | yes - forecast rows include tide fields and preferred-tide beach fields | partial - rendered via detail tabs or companion utility page, not the surf-call row itself | partial - `ForecastDataSourceIndicator` exists, but spot page does not yet render a source-chip contract | yes - `getBeachCameraUrl` and `LiveCamSchema` when camera exists | yes - reviews, intel, sessions tabs and `getBeachReviews` | yes - canonicalized beach fit fields feed `computeSurfCall` and `selectBestWindow` | partial - `InlineSignupCta`, `StickySignupBar`, canonical web URL; no exact app window link |
...
- `getSpotSurfReport` - authenticated surf-call path with user preferences and
  skill-level adjustment; Phase 15 should preserve this personalization boundary.
...
| forecast fetch | `actions/forecast/intent-forecast-actions.ts`, `actions/spot/spot-surf-report-actions.ts` | Live surf-window UI could add redundant forecast reads or widen existing query horizons. | document only | Phase 15 must reuse existing today/tomorrow and 168-hour fetch paths or justify a measured query change. |
...
| cache hit/miss behavior | `lib/utils/forecast-hub-utils.ts`, `actions/spot/spot-surf-report-actions.ts` | Recommendation modules can look cheap on warm cache and expensive on cold cache. | document only | Later phases must record warm-cache and cold-cache behavior for pilot routes before rollout. |
