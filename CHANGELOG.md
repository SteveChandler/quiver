...
- **Comprehensive Swell Window Audit & Fix** (uncommitted) - Automated swell window derivation for all 252 beaches with terrain ray-tracing data via `scripts/derive-swell-windows.ts`. Windows now derived from `swell_access_factors` (threshold >= 0.15) using contiguous-bin algorithm with wrap-around handling. Enabled terrain-aware scoring (`terrain_enabled=true`) for 202 additional beaches. Previously only 59 CA beaches had terrain scoring; now all 261 beaches with terrain data use both swell windows AND terrain access factors for directional scoring. Excluded 5 fully-blocked and 4 narrow-window beaches pending terrain re-analysis.
