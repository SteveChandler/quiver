...
- **Growth: `BeachAlertCta` component on beach detail actions** — Created `components/beach-detail/beach-alert-cta.tsx`, a Bell-icon CTA placed after the Log/Plan Session buttons in `beach-actions.tsx`. For non-authenticated users, clicking stores a `pendingAction` (`type: "alert"`) via `usePendingAction` then opens `UnifiedAuthModal` in `signup` mode with source `"beach-alert-cta"` and a contextMessage inviting the user to get surf alerts. For authenticated users, clicking shows an inline success message directing them to Profile Settings (actual notification wiring is a follow-up task). 8 TDD tests cover Bell icon render, auth modal opening, pending action storage, and authenticated success message.
