...
- **Intel Tab State Leakage**: Fixed issue where Intel tab's map view would persist/interfere with Forecast tab when switching tabs. Updated `components/home-screen/index.tsx` to conditionally render ALL tabs only when active (forecast, nearby, community), forcing proper unmounting of components (including Intel map) when switching tabs. Added `z-10` to all TabsContent components and `z-0` to Intel map container for proper stacking. Added unique key prop to IntelMap for proper cleanup on filter changes.
