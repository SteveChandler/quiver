...
- **Map Marker Wave Heights Truncated by Row Limit** - Bulk forecast API fetched 12 days of forecasts (~4,700 rows for 50 beaches), exceeding Supabase's 1,000-row default limit. Beaches with later-sorting UUIDs silently lost all forecast data, showing "0-1ft". Now fetches only today + tomorrow (~800 rows), matching the server action pattern.
