"""Train v2 bias correction model with monotone constraints and bucket validation."""
import argparse
import os
import sys
...
def main():
    parser = argparse.ArgumentParser(description='Train v2 bias correction model')
    parser.add_argument('--data', default='data/training_data_v2.csv',
                        help='Path to training data CSV')
    parser.add_argument('--output', default='models/bias_model_v2.json',
    ...
    print("=" * 60)
    print("Quiver ML v2 Bias Model Training")
    print("=" * 60)
    ...
    # Monotone constraint: forecast_height_m gets -1 (as forecast increases, correction decreases)
    # All other features get 0 (no constraint)
    monotone_constraints = [
        -1 if col == 'forecast_height_m' else 0
    ...
    # 7. Holdout evaluation with guardrails
    print("\n7. Holdout evaluation...")

    predicted_bias = model.predict(X_holdout)
    raw_forecast = df_holdout['forecast_height_m'].values

    # Apply same guardrails as production (from model.py)
    max_bias = np.maximum(np.abs(raw_forecast) * 0.5, 0.3)
