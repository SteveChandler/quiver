...
The signed-in home hero draws the combined field (primary, secondary, wind) in the dark-stage palette on a 2D canvas (`components/oracle/zine/hero-swell-field.tsx`). It shares:

- **The look:** `particle-style.ts` (counts, spacing, dash size, speed, fades, the combined-view counts and wind drift), plus `SWELL_FIELD_PARTICLE_COLOR_DARK_STAGE` and `SWELL_FIELD_DARK_STAGE_DASH_LENGTH_SCALE`.
