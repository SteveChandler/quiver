...
export function BadgeGallery({ badges, userBadges = [], className }: BadgeGalleryProps) {
  const [selectedCategory, setSelectedCategory] = useState<'all' | 'global' | 'journal' | 'quiver'>('all');
  ...
  const categoryStats = {
    global: {
      total: (organizedBadges.global?.unlocked.length || 0) + (organizedBadges.global?.locked.length || 0),
      unlocked: organizedBadges.global?.unlocked.length || 0,
    },
    journal: {
      total: (organizedBadges.journal?.unlocked.length || 0) + (organizedBadges.journal?.locked.length || 0),
      unlocked: organizedBadges.journal?.unlocked.length || 0,
    },
    quiver: {
      total: (organizedBadges.quiver?.unlocked.length || 0) + (organizedBadges.quiver?.locked.length || 0),
  ...
  const renderBadgeGrid = (categoryBadges: { unlocked: BadgeData[]; locked: BadgeData[] }) => {
    ...
    <Card className={className}>
      <CardHeader>
        ...
        <div className="flex items-center gap-4 text-sm text-muted-foreground">
          <span>{totalStats.unlocked} / {totalStats.total} badges unlocked</span>
          <Badge variant="secondary">
            {Math.round((totalStats.unlocked / totalStats.total) * 100)}% complete
          </Badge>
        </div>
      </CardHeader>
      ...
      <CardContent>
        <Tabs value={selectedCategory} onValueChange={(value) => setSelectedCategory(value as any)}>
          ...
          <TabsContent value="journal" className="mt-6">
            {organizedBadges.journal && renderBadgeGrid(organizedBadges.journal)}
          </TabsContent>
