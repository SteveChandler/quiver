...
export function PublicForecastAnswer({
  ...
  const selectedWindow = normalizeForecastWindowParam(searchParams?.get("window"));
  const selectedEnd = normalizeForecastWindowParam(searchParams?.get("windowEnd"));
  const dateParam = searchParams?.get("date");
  const selectedDate = dateParam && /^\d{4}-\d{2}-\d{2}$/.test(dateParam) && Number.isFinite(Date.parse(dateParam)) ? dateParam : null;
