...
function UnifiedSurfCard({
  ...
  // Format ISO time to readable time in beach timezone
  ...
  const updatedTime = useMemo(() => {
    try {
      const date = new Date(surfCall.updatedAt);
      return date.toLocaleTimeString("en-US", {
        hour: "numeric",
        minute: "2-digit",
        timeZone: timezone,
  ...
  const windowStart = formatIsoTime(surfCall.bestWindowStart);
  const windowEnd = formatIsoTime(surfCall.bestWindowEnd);
  const peakTime = formatIsoTime(surfCall.peakTime);
