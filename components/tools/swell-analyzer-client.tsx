...
export function SwellAnalyzerClient({ initialData, isDemo = false }: SwellAnalyzerClientProps) {
  ...
  const [swellDirection, setSwellDirection] = useState(initialData?.currentSwell?.direction ?? 270);
  ...
  const displayHeight = mode === "current" && beachData?.currentSwell?.height != null
    ...
    : swellHeight;
  const displayPeriod = mode === "current" && beachData?.currentSwell?.period != null
    ? beachData.currentSwell.period
    : swellPeriod;
  const displayDirection = mode === "current" && beachData?.currentSwell?.direction != null
  ...
  const beachSwellMatch = mode === "current"
              ...
              <div className="flex items-center gap-3 shrink-0">
                ...
                >
                  <button
                    ...
                    style={
                      mode === "current"
                        ? { background: "rgba(247,142,66,0.15)", color: "#F78E42" }
                        : { color: "#7A8CC0" }
                    }
                  ...
                  {mode === "current" && beachData?.currentSwell ? (
                    ...
                    <div className="space-y-3">
                      <p className="text-sm text-[#7A8CC0]">
                        No current forecast data available. Switch to Explore to analyze hypothetical swells.
                      </p>
                    </div>
                  ) : (
