...
-- Based on PostgreSQL query performance analysis showing:
-- - 88,315 beach_reviews queries (0.5% time)
-- - 4,336 beaches pagination queries (1.4% time) 
-- - 2,462 sessions lateral join queries (0.6% time)
...
-- Covers: beach_reviews WHERE beach_id = ANY($1) with rating sorts
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_beach_reviews_beach_id_rating 
ON beach_reviews (beach_id, overall_rating DESC NULLS LAST);

-- Composite index for beach review aggregations
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_beach_reviews_beach_stats 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_beaches_name_asc 
ON beaches (name ASC NULLS LAST);

-- Covering index for beach listings (avoid table lookups)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_beaches_list_covering 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_beaches_location_lat 
ON beaches (latitude)
WHERE latitude IS NOT NULL;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_beaches_location_lng 
ON beaches (longitude)
WHERE longitude IS NOT NULL;

-- Composite index for bounding box queries
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_beaches_location_composite 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_sessions_public_recent 
ON sessions (created_at DESC) 
WHERE is_public = true;

-- Beach sessions optimization  
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_sessions_beach_status_recent 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_sessions_beach_id 
ON sessions (beach_id) WHERE beach_id IS NOT NULL;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_sessions_board_id 
ON sessions (board_id) WHERE board_id IS NOT NULL;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_sessions_profile_id 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_profiles_id_name_avatar 
ON profiles (id, full_name, avatar_url);

-- Boards for session joins  
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_boards_id_name 
ON boards (id, name);

-- Comments count optimization
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_comments_session_id 
ON comments (session_id) WHERE session_id IS NOT NULL;

-- User follows for realtime optimization
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_user_follows_following_follower 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_sessions_rated 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_sessions_recent_activity 
...
-- Run this after deployment to validate improvements:
-- SELECT * FROM v_index_usage_stats WHERE tablename IN ('beaches', 'sessions', 'beach_reviews');

-- ================================================
-- 8. Comments for Future Reference
-- ================================================

/*
Expected Performance Improvements:
1. Beach reviews: 88,315 calls should see 60-80% speed improvement
2. Beach pagination: 4,336 calls should see 40-60% speed improvement  
3. Session queries: 2,462 calls should see 30-50% speed improvement

Monitor these metrics post-deployment:
- Query execution time reduction
- Index scan vs seq scan ratios
- Overall database CPU utilization
- PostgREST response times

If indexes grow too large or cause write performance issues:
- Consider partial indexes with more restrictive WHERE clauses
- Drop unused indexes using pg_stat_user_indexes
- Implement index maintenance jobs for high-write tables
*/ 