...
function generateConditionDescription(user: MockUser, beachName: string, beachType: string): string {
  ...
  const descriptions = {
    rookie: [
      `${style.phrases[0]} The waves at ${beachName} are perfect right now! Clean faces and super manageable size. Water feels amazing and everyone out there is so friendly. This is exactly what I dreamed surfing would be like!`,
      `Just got out of the water at ${beachName} and I'm still buzzing! Conditions are incredible - gentle waves with crystal clear water. Perfect for someone still learning like me. ${style.phrases[1]}`,
      `${beachName} is firing today! Okay maybe not firing like the pros say, but perfect for me. Small, clean waves with no scary currents. Even managed to catch a few on the inside. ${style.phrases[4]}`
    ],
    
    local: [
      `${style.phrases[0]} - ${beachName} has consistent 3-4ft waves with light offshore winds. Water temp is around 65-66°F. Good conditions should hold through the afternoon before the usual wind picks up. Classic setup for this time of year.`,
      `Solid session at ${beachName} this morning. Clean 2-3ft waves with glassy conditions until about 10am. ${style.phrases[1]}: get out early before the afternoon onshore kicks in. Water clarity is excellent right now.`,
    ...
    traveler: [
      `${beachName} conditions remind me of Indo but with colder water! Clean 4ft waves with perfect offshore winds. ${style.phrases[3]} - this place has such a unique character. Wave quality is surprisingly good for this region.`,
      `First session at ${beachName} and the conditions are stellar! ${style.phrases[0]} the reefs back home, but the wave shape is incredibly consistent here. Long rides and forgiving sections - perfect for exploring a new break.`,
      `${beachName} absolutely delivering today! Conditions are different from my usual spots but in the best way. Consistent sets with beautiful blue water. ${style.phrases[2]} this quality at a beach break.`
    ],
    
    photographer: [
      `${style.phrases[0]} at ${beachName} today for both surfing and shooting! Morning light is incredible, highlighting every wave perfectly. Conditions are clean with beautiful color contrast between the water and sky. ${style.phrases[3]} moments everywhere.`,
      `${beachName} conditions are visually stunning right now! Clean waves with amazing light penetration through the water. ${style.phrases[2]} reflecting off the wave faces creating incredible photo opportunities. Both the action and scenery are perfect.`,
      `Perfect storm of conditions at ${beachName}! Great waves combined with ${style.phrases[0]} make this a photographer's dream. Every set offers something different to capture. The natural beauty here is unmatched.`
    ],
    
    tactical: [
      `${style.phrases[0]}: ${beachName} conditions optimal for aquatic operations. Wave height 3-4 feet, period 10-12 seconds, winds 5-8 knots offshore. Water temperature 65°F. ${style.phrases[3]} identified for extended operations.`,
      `Field report: ${beachName} environmental conditions suitable for tactical deployment. Swell direction northwest at optimal angle. Minimal civilian interference observed. ${style.phrases[2]} for mission execution.`,
      `${beachName} reconnaissance complete. Current conditions show strategic advantages with consistent wave patterns. ${style.phrases[1]}: approach from south side for optimal positioning. Mission parameters met.`
    ],
    
    competitor: [
      `${beachName} conditions are next level today! Perfect waves for performance training - consistent sets with rippable faces. This is exactly what I need for competition prep. ${style.phrases[2]} training ground right now.`,
      `Training session at ${beachName} exceeded expectations! Wave quality is competition-level with multiple maneuver sections. Conditions are pushing my performance to new heights. ${style.phrases[0]} for serious surfers.`,
...
function generateParkingDescription(user: MockUser, beachName: string, beachType: string): string {
  const parkingDescriptions = {
    ...
    local: [
      `Heads up - ${beachName} main lot is full by 9am on weekends. Try the residential streets on the south side, about a 3-minute walk to the beach. Meters take cards now which is convenient. Peak season parking is always tough here.`,
      ...
      `Pro tip for ${beachName} parking: there's a hidden lot behind the surf shop that's usually less crowded. $5 all day vs $2/hour at the main lot. Plus they validate if you buy anything from the shop.`
    ],
...
function generateCrowdDescription(user: MockUser, beachName: string, beachType: string): string {
  const crowdDescriptions = {
    ...
    local: [
      `${beachName} getting pretty crowded this week with the good conditions. About 30-40 people out at peak times. Most folks are respectful but heads up for some newer faces who might not know the lineup yet. Overall vibe is still good though.`,
...
function generateHazardDescription(user: MockUser, beachName: string, beachType: string): string {
  const hazardDescriptions = {
    ...
    local: [
      `Heads up everyone - strong rip current on the south side of ${beachName} today. It's actually great for getting out back but be aware of less experienced surfers who might get caught. As always, use it to your advantage but keep an eye on others.`,
    ...
    tactical: [
      ...
      `Field report: ${beachName} shows elevated risk factors due to equipment hazards from inexperienced operators. Recommend maintaining safe operational distance and continuous threat assessment. Mission success requires adaptive tactical response.`
    ]
  };
...
async function seedBeachReviews(
  ...
  for (const beach of beaches) {
    // Generate 1-3 reviews per beach from random NPCs
    ...
    for (const user of selectedUsers) {
      ...
      const visitDate = randomDateInRange(21); // Within last 3 weeks
      const createdAt = randomDateInRange(21); // Within last 3 weeks
...
async function seedIntelPosts(
  ...
  for (const beach of beaches) {
    // Generate 1-3 intel posts per beach from random NPCs
    ...
    for (const user of selectedUsers) {
      ...
      const createdAt = randomDateInRange(10); // Within last 10 days
