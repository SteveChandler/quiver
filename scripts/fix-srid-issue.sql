-- Fix SRID (coordinate reference system) issues in PostGIS functions
-- Run this in your Supabase SQL editor

-- Function to find nearby buoys (FIXED - proper SRID handling)
...
BEGIN
  ...
  SELECT 
    ...
    ST_Distance(b.coordinates, ST_SetSRID(ST_Point(lng, lat), 4326)::geography) as distance_meters,
    degrees(ST_Azimuth(
      ST_SetSRID(ST_Point(lng, lat), 4326)::geometry, 
      ST_SetSRID(b.coordinates::geometry, 4326)
    )) as direction_degrees
  FROM buoys b
  WHERE b.active = true
    AND b.coordinates IS NOT NULL
    AND ST_Distance(b.coordinates, ST_SetSRID(ST_Point(lng, lat), 4326)::geography) <= max_distance_meters
  ORDER BY distance_meters ASC
  LIMIT limit_count;
END;
$$ LANGUAGE plpgsql;

-- Function to consolidate buoy conditions (uses the fixed get_nearby_buoys)
...
-- Test the functions
SELECT 'SRID issues fixed successfully!' as status; 