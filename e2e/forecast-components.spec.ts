...
test.describe("Forecast Components", () => {
  ...
  test.describe("ForecastTab Component", () => {
    ...
    test("handles high confidence forecast display", async ({ page }) => {
      ...
      const confidenceIndicators = page.locator('[data-testid*="confidence"], text=/confidence/i');
    ...
    test("navigates to beach details when forecast is clicked", async ({ page }) => {
      // Look for clickable forecast elements
      const forecastCards = page.locator('[data-testid="beach-card"], .forecast-card, .beach-forecast').first();
  ...
  test.describe("ForecastFeedbackForm Component", () => {
    ...
    test("displays feedback form elements", async ({ page }) => {
      ...
      const feedbackTrigger = page.locator('button:has-text("feedback"), button:has-text("Feedback")').first();
    ...
    test("allows wave height slider interaction", async ({ page }) => {
      ...
      const logSessionButton = page.locator('button:has-text("Log Session"), a:has-text("Log Session")').first();
      ...
      if (await logSessionButton.count() > 0) {
        ...
        if (await sliders.count() > 0) {
          ...
          const sliderValue = await waveHeightSlider.getAttribute("aria-valuenow");
    ...
    test("validates form submission", async ({ page }) => {
      ...
      if (await logSessionButton.count() > 0) {
        ...
        const beachInput = page.locator('input[placeholder*="beach"], input[placeholder*="location"]').first();
        ...
        if (await submitButton.count() > 0) {
          ...
          const hasSuccess = await page.locator('text=/success|saved|logged/i').count() > 0;
          const hasValidation = await page.locator('text=/required|error/i').count() > 0;
  ...
  test.describe("Adjusted Forecast Display", () => {
    ...
    test("displays forecast adjustments when available", async ({ page }) => {
      ...
      const adjustedIndicators = page.locator('text=/adjusted|community|confirmed/i');
    ...
    test("shows tooltip information for forecast details", async ({ page }) => {
      ...
      const infoButtons = page.locator('button[aria-label*="info"], button:has([class*="info"]'), .tooltip-trigger').first();
  ...
  test.describe("Forecast Navigation", () => {
    test("allows navigation between different forecast views", async ({ page }) => {
      // Look for forecast navigation tabs or buttons
      const forecastTabs = page.locator('[role="tab"], .tab, button:has-text("forecast")');
    ...
    test("handles date navigation in forecasts", async ({ page }) => {
      ...
      const dateNavigators = page.locator('button:has-text("next"), button:has-text("previous"), .date-nav');
