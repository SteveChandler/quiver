...
const migration = readFileSync(
  join(process.cwd(), "supabase/migrations/20260818120000_surf_alert_trust_invariants.sql"),
  "utf8",
);
...
describe("surf alert trust invariant migration", () => {
  it("keeps producer dedupe permanent and refuses duplicate history without deleting it", () => {
    ...
    expect(migration).toContain("RAISE EXCEPTION 'home_morning_call dedupe duplicates exist");
    expect(migration).not.toMatch(/DELETE\s+FROM\s+public\.notification_events/i);
  });
  ...
  it("stages install-scoped identity without a destructive global token retrofit", () => {
    ...
    expect(migration).toContain("DROP INDEX IF EXISTS public.user_devices_active_installation");
    expect(migration).toContain("ON CONFLICT (installation_id) WHERE installation_id IS NOT NULL AND retired_at IS NULL");
    ...
    expect(migration).toContain("installation_id IS NOT DISTINCT FROM p_installation_id");
    expect(migration).toContain("installation_id IS NOT NULL AND retired_at IS NULL");
    ...
    expect(migration).toContain("installation_id = p_installation_id OR device_token = p_device_token");
