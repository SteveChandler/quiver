...
describe('GET /api/surf/week-scout/snapshots/:snapshotId', () => {
  const originalFlag = process.env.WEEKEND_SCOUT_ENABLED;
  ...
  afterAll(() => {
    if (originalFlag === undefined) delete process.env.WEEKEND_SCOUT_ENABLED;
    else process.env.WEEKEND_SCOUT_ENABLED = originalFlag;
  });
