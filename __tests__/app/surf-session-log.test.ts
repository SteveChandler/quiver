...
import { metadata } from "@/app/surf-session-log/page";

describe("Surf session log SEO page", () => {
  const pageSource = readFileSync(
    join(process.cwd(), "app/surf-session-log/page.tsx"),
    "utf8",
  );
  ...
  it("is included in the static sitemap routes", () => {
    ...
    expect(sitemapSource).toContain('"/surf-session-log"');
  });
  ...
  it("carries the target query in the H1", () => {
    ...
    expect(h1?.[1]).toBe("Best App for Surf Session Logging");
    expect(h1?.[1].toLowerCase()).toContain("best app for surf session logging");
  });
});
