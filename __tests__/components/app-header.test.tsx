...
// Mock session invitations subscription
jest.mock("@/hooks/use-session-invitations-subscription", () => ({
  useSessionInvitationsSubscription: jest.fn(),
}));
...
describe("AppHeader", () => {
  ...
  /**
   * Phase 3: Notification Bell Extraction Tests
   *
   * Tests the notification bell that was extracted from the dropdown menu:
   * - Standalone bell button rendering
   ...
   */
  describe("Phase 3: Notification Bell", () => {
    describe("Notification Bell Rendering", () => {
      ...
      it("renders notification bell button for authenticated users", () => {
        render(<AppHeader />);
      ...
      it("renders bell icon inside button", () => {
        ...
        const bellIcon = screen.getByTestId("notification-bell-icon");
        expect(bellIcon).toBeInTheDocument();
      });

      it("does not render notification bell for guest users", () => {
        (useAuth as jest.Mock).mockReturnValue({
          user: null,
          isLoading: false,
          signOut: jest.fn(),
        ...
        const bellButton = screen.queryByTestId("notification-bell-button");
        expect(bellButton).not.toBeInTheDocument();
      });

      it("bell button is wrapped in link to /inbox", () => {
        render(<AppHeader />);

        const bellButton = screen.getByTestId("notification-bell-button");
        const link = bellButton.closest("a");

        expect(link).toHaveAttribute("href", "/inbox");
      });

      it("bell button has proper styling classes", () => {
        render(<AppHeader />);

        const bellButton = screen.getByTestId("notification-bell-button");
        expect(bellButton.className).toMatch(/h-8/);
        expect(bellButton.className).toMatch(/w-8/);
        expect(bellButton.className).toMatch(/rounded-full/);
        expect(bellButton.className).toMatch(/hover:bg-muted/);
        expect(bellButton.className).toMatch(/transition-colors/);
        expect(bellButton.className).toMatch(/duration-200/);
      });
    });
    ...
    describe("Badge Display Logic", () => {
      ...
      it("shows exact count for numbers 1-9", () => {
        (useDataFetcher as jest.Mock).mockReturnValue({
          data: 5,
          refetch: jest.fn(),
        });
      ...
      it("badge has destructive variant styling", () => {
        (useDataFetcher as jest.Mock).mockReturnValue({
          data: 1,
          refetch: jest.fn(),
        });
      ...
      it("badge has absolute positioning classes", () => {
        (useDataFetcher as jest.Mock).mockReturnValue({
          data: 1,
          refetch: jest.fn(),
        });
        ...
        expect(badge.className).toMatch(/-top-1/);
        expect(badge.className).toMatch(/-right-1/);
      });

      it("badge has border for contrast", () => {
        (useDataFetcher as jest.Mock).mockReturnValue({
          data: 1,
          refetch: jest.fn(),
        });
        ...
        expect(badge.className).toMatch(/border-background/);
      });

      it("badge has appropriate sizing classes", () => {
        (useDataFetcher as jest.Mock).mockReturnValue({
          data: 1,
          refetch: jest.fn(),
        });
        ...
        expect(badge.className).toMatch(/min-w-5/);
        expect(badge.className).toMatch(/px-1/);
    ...
    describe("Navigation Functionality", () => {
      ...
      it("clicking bell navigates to /inbox", async () => {
        const user = userEvent.setup();
        render(<AppHeader />);
      ...
      it("bell link does not open in new tab", () => {
        render(<AppHeader />);
    ...
    describe("Accessibility", () => {
      ...
      it("has aria-label 'Notifications' when count = 0", () => {
        (useDataFetcher as jest.Mock).mockReturnValue({
          data: 0,
          refetch: jest.fn(),
        });
      ...
      it("has aria-label with count when count > 0", () => {
        (useDataFetcher as jest.Mock).mockReturnValue({
          data: 3,
          refetch: jest.fn(),
        });
      ...
      it("aria-label shows exact count for large numbers", () => {
        (useDataFetcher as jest.Mock).mockReturnValue({
          data: 25,
          refetch: jest.fn(),
        });
        ...
        const bellButton = screen.getByTestId("notification-bell-button");
        // ARIA label should have full count, badge shows 9+
        expect(bellButton).toHaveAttribute(
          "aria-label",
          "Notifications, 25 unread"
        );
      ...
      it("bell button is keyboard accessible", async () => {
        ...
        expect(bellButton.tabIndex).toBeGreaterThanOrEqual(0);
      });

      it("bell icon has appropriate color classes", () => {
        render(<AppHeader />);

        const bellIcon = screen.getByTestId("notification-bell-icon");
        const classNames = bellIcon.getAttribute("class") || "";
        expect(classNames).toMatch(/text-foreground\/70/);
        expect(classNames).toMatch(/hover:text-foreground/);
        expect(classNames).toMatch(/transition-colors/);
      });
    });
    ...
    describe("Dropdown Menu Integration", () => {
      ...
      it("notification item removed from dropdown menu", () => {
        render(<AppHeader />);

        const dropdown = screen.queryByTestId("dropdown-menu");

        // Dropdown should not contain notification link/text
        if (dropdown) {
          const notificationText = Array.from(dropdown.querySelectorAll("*"))
            .map((el) => el.textContent)
            .join(" ")
            .toLowerCase();

          // May still have "Notifications" text from test, but shouldn't have bell icon in dropdown
          const bellIconsInDropdown =
            dropdown.querySelectorAll('[data-testid="notification-bell-icon"]');
          expect(bellIconsInDropdown.length).toBe(0);
        }
      });

      it("dropdown still has Profile menu item", () => {
        render(<AppHeader />);
      ...
      it("dropdown still has Log out menu item", () => {
        render(<AppHeader />);
    ...
    describe("Real-time Updates", () => {
      ...
      it("uses useDataFetcher for unread count", () => {
        render(<AppHeader />);

        // useDataFetcher should be called
        expect(useDataFetcher).toHaveBeenCalled();
      });

      it("badge updates when count changes", () => {
      ...
      it("badge appears when count goes from 0 to 1", () => {
        const mockRefetch = jest.fn();
        (useDataFetcher as jest.Mock).mockReturnValue({
          data: 0,
          refetch: mockRefetch,
        ...
        let badge = screen.queryByTestId("notification-badge");
        expect(badge).not.toBeInTheDocument();
      ...
      it("badge disappears when count goes from 1 to 0", () => {
        const mockRefetch = jest.fn();
        (useDataFetcher as jest.Mock).mockReturnValue({
          data: 1,
          refetch: mockRefetch,
        ...
        badge = screen.queryByTestId("notification-badge");
        expect(badge).not.toBeInTheDocument();
      });
    });
    ...
    describe("Layout Integration", () => {
      ...
      it("bell positioned in right section of header", () => {
        render(<AppHeader />);

        const bellButton = screen.getByTestId("notification-bell-button");
        const header = bellButton.closest("header");

        // Bell should be in right section (ml-auto container)
        const rightSection = bellButton.closest('[class*="ml-auto"]');
        expect(rightSection).toBeInTheDocument();
      });

      it("bell positioned before user avatar", () => {
        render(<AppHeader />);
        ...
        const bellContainer = children.find((child) =>
          child.contains(bellButton)
        );
        const avatarContainer = children.find((child) => child.contains(avatar));

        if (bellContainer && avatarContainer) {
          const bellIndex = children.indexOf(bellContainer);
          const avatarIndex = children.indexOf(avatarContainer);
          expect(bellIndex).toBeLessThan(avatarIndex);
        }
      });
      ...
      it("does not break navigation links layout", () => {
        render(<AppHeader />);
  ...
  describe("Phase 4: Enhanced Styling & Polish", () => {
    ...
    describe("Accessibility - ARIA Labels", () => {
      ...
      it("notification bell has aria-label", () => {
        ...
        if (notificationButton) {
          ...
          expect(notificationButton.getAttribute("aria-label")).toContain(
            "Notification"
          );
        }
      });
    ...
    describe("Integration - Phase 4 does not break previous phases", () => {
      ...
      it("notification bell still works with enhanced avatar", () => {
        render(<AppHeader />);
