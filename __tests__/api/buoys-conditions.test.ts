...
import { GET } from "@/app/api/buoys/conditions/route";
...
describe("GET /api/buoys/conditions", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it("uses the shared API wrapper module for response helpers", () => {
    const source = readFileSync(
      join(process.cwd(), "app/api/buoys/conditions/route.ts"),
      "utf8"
    );
  ...
  it("validates required coordinates before creating a Supabase client", async () => {
    const response = await GET(
      new NextRequest("http://localhost:3000/api/buoys/conditions?lat=37.7")
    );
  ...
  it("queries the nearest buoy and maps condition measurements", async () => {
    ...
    const response = await GET(
      new NextRequest(
        "http://localhost:3000/api/buoys/conditions?latitude=37.7&longitude=-122.5"
      )
    );
    const body = await response.json();

    expect(response.status).toBe(200);
    expect(mockRpc).toHaveBeenCalledWith("get_nearest_buoy_with_conditions", {
      target_lat: 37.7,
      target_lng: -122.5,
  ...
  it("returns a plain 404 response when no active buoy is found", async () => {
    const consoleLog = jest.spyOn(console, "log").mockImplementation(() => undefined);
    mockRpc.mockResolvedValue({ data: [], error: null });

    const response = await GET(
      new NextRequest(
        "http://localhost:3000/api/buoys/conditions?lat=37.7&lon=-122.5"
      )
    );
    const body = await response.json();

    expect(response.status).toBe(404);
    expect(body).toEqual({ error: "No active buoy found for location" });

    consoleLog.mockRestore();
  });

  it("returns fallback mock conditions when the buoy RPC errors", async () => {
    const consoleLog = jest.spyOn(console, "log").mockImplementation(() => undefined);
    ...
    const response = await GET(
      new NextRequest("http://localhost:3000/api/buoys/conditions?lat=37.7&lon=-122.5")
    );
    const body = await response.json();
  ...
  it("wraps unexpected setup errors", async () => {
    ...
    expect(body).toMatchObject({
      success: false,
      error: "Error fetching buoy conditions",
    });
