...
describe("IndexNow URL collectors", () => {
  ...
  it("includes forecast, cam, guide, and generated SEO funnel families", () => {
    ...
    expect(urls.some((url) => url.startsWith(`${SITE_URL}/surf-cams/`))).toBe(true);
    expect(urls).toContain(`${SITE_URL}/surf-cams/southern-california`);
    // Redirecting regional cam URLs must never be submitted to IndexNow.
    expect(urls).not.toContain(`${SITE_URL}/cams/southern-california`);
    expect(urls).not.toContain(`${SITE_URL}/cams/florida`);
    expect(urls).not.toContain(`${SITE_URL}/cams/hawaii`);
    expect(urls).toContain(`${SITE_URL}/surf-cams/hawaii`);
