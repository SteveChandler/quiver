...
import { runProOfferAutomation, refreshLifecycleEligibility } from '@/lib/subscription/offer-automation';
...
it('automatically claims the due accepted reward and persists retry scheduling', async () => {
  mockRpc.mockImplementation(async name => name === 'enroll_automatic_pro_offers' ? 1 : name === 'pro_offer_fulfillment_queue' ? [{ user_id, award_id }] : null);
