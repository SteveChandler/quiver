...
describe('runWeekendScoutCron', () => {
  const originalEnabled = process.env.WEEKEND_SCOUT_ENABLED;
  const originalAllowlist = process.env.WEEKEND_SCOUT_TEST_USER_IDS;

  beforeEach(() => {
    process.env.WEEKEND_SCOUT_ENABLED = 'true';
    delete process.env.WEEKEND_SCOUT_TEST_USER_IDS;
  });

  afterAll(() => {
    if (originalEnabled === undefined) delete process.env.WEEKEND_SCOUT_ENABLED;
    else process.env.WEEKEND_SCOUT_ENABLED = originalEnabled;
    if (originalAllowlist === undefined) delete process.env.WEEKEND_SCOUT_TEST_USER_IDS;
    else process.env.WEEKEND_SCOUT_TEST_USER_IDS = originalAllowlist;
  });
  ...
  it('returns a skipped summary while the kill switch is off', async () => {
    delete process.env.WEEKEND_SCOUT_ENABLED;
    const deps = dependencies();
    const response = await runWeekendScoutCron(new Request('http://localhost/cron'), deps);
  ...
  it('filters preferences, allowlist, location freshness, and local Friday noon before ranking', async () => {
    process.env.WEEKEND_SCOUT_TEST_USER_IDS = 'user-1,user-stale,user-missing,user-wrong-time';
    const deps = dependencies();
