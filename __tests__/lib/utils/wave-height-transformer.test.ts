...
describe('Wave Height Transformer', () => {
  ...
  describe('calculatePeriodFactor', () => {
    ...
    it('should calculate correctly for long periods', () => {
      ...
      // 16s: 1.0 + (16 - 10) * 0.05 = 1.3
      expect(calculatePeriodFactor(16)).toBeCloseTo(1.3, 5);
      // 18s: 1.0 + (18 - 10) * 0.05 = 1.4
      expect(calculatePeriodFactor(18)).toBeCloseTo(1.4, 5);
    });
    ...
    it('should clamp at maximum (1.4)', () => {
      // 20s: would be 1.5, but clamped to 1.4
      expect(calculatePeriodFactor(20)).toBe(1.4);
      // 25s: clamped to 1.4
      expect(calculatePeriodFactor(25)).toBe(1.4);
    });
  });
  ...
  describe('transformToFaceHeight', () => {
    describe('Base shoaling only (no period/direction modifiers)', () => {
      ...
      it('should round to 1 decimal place', () => {
        // 1.55ft @ 10s = 1.55 * 1.6 = 2.48, rounded to 2.5
        const result = transformToFaceHeight({
          rawHeightFt: 1.55,
          periodS: 10,
          swellDirectionDeg: null,
    ...
    describe('Period amplification', () => {
      it('should amplify for long period swell', () => {
        // 2.0ft @ 16s: 2.0 * 1.6 * 1.3 = 4.16, rounded to 4.2
        const result = transformToFaceHeight({
          rawHeightFt: 2.0,
          periodS: 16,
          swellDirectionDeg: null,
      ...
      it('should reduce for short period wind chop', () => {
        // 2.0ft @ 6s: 2.0 * 1.6 * 0.8 = 2.56, rounded to 2.6
        const result = transformToFaceHeight({
          rawHeightFt: 2.0,
          periodS: 6,
          swellDirectionDeg: null,
      ...
      it('should use reference period when null', () => {
        // 2.0ft @ null (10s): 2.0 * 1.6 * 1.0 = 3.2
        const result = transformToFaceHeight({
          rawHeightFt: 2.0,
          periodS: null,
          swellDirectionDeg: null,
    ...
    describe('Direction factor with terrain', () => {
      ...
      it('should apply reduced direction factor for blocked access', () => {
        // 2.0ft @ 10s, direction factor 0.6: 2.0 * 1.6 * 1.0 * 0.6 = 1.92, rounded to 1.9
        const result = transformToFaceHeight({
          rawHeightFt: 2.0,
          periodS: 10,
          swellDirectionDeg: 180,
    ...
    describe('Combined factors - realistic scenarios', () => {
      ...
      it('should calculate moderate swell with partial blocking', () => {
        ...
        // 3.0 * 1.6 * 1.2 * 0.8 = 4.608, rounded to 4.6
        const result = transformToFaceHeight({
          rawHeightFt: 3.0,
          periodS: 14,
      ...
      it('should handle short period wind chop with poor access', () => {
        ...
        // 1.5 * 1.6 * 0.85 * 0.6 = 1.224, rounded to 1.2
        const result = transformToFaceHeight({
          rawHeightFt: 1.5,
          periodS: 7,
    ...
    describe('Edge cases', () => {
      ...
      it('should handle very small heights', () => {
        // 0.1ft @ 10s = 0.1 * 1.6 = 0.16, rounded to 0.2
        const result = transformToFaceHeight({
          rawHeightFt: 0.1,
          periodS: 10,
          swellDirectionDeg: null,
  ...
  describe('getTransformationFactors', () => {
    it('should return all transformation factors', () => {
      ...
      expect(factors.baseShoaling).toBe(1.6);
      expect(factors.periodFactor).toBeCloseTo(1.2, 5);
      expect(factors.directionFactor).toBe(1.0);
      // 2.0 * 1.6 * 1.2 * 1.0 = 3.84, rounded to 3.8
      expect(factors.faceHeightFt).toBe(3.8);
    });
