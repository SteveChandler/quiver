...
describe('Wave Height Transformer', () => {
  ...
  describe('transformToFaceHeightWithMetadata — CDIP direction shadow', () => {
    ...
    it('uses the continuous direction term only when the beach flag is on', () => {
      const beach = {
        shoaling_factors: BLACKS_BUCKETS,
        swell_window_center_deg: 282.5,
        swell_window_halfwidth_deg: 102.5,
        swell_access_factors: accessArray({ [toBin5(202)]: 0.01 }),
      };
      const oldResult = transformToFaceHeightWithMetadata({
        rawHeightFt: 2,
        periodS: 14,
        swellDirectionDeg: 202,
        source: 'cdip_sig',
        beach,
        directionScoringEnabled: false,
      });
      const newResult = transformToFaceHeightWithMetadata({
        rawHeightFt: 2,
        periodS: 14,
        swellDirectionDeg: 202,
        source: 'cdip_sig',
        beach,
        directionScoringEnabled: true,
      });

      expect(oldResult.faceHeightFt).toBe(4.3);
      expect(oldResult.directionFactor).toBe(1);
      expect(newResult.directionFactor).toBeLessThan(1);
      expect(newResult.faceHeightFt).toBeLessThan(oldResult.faceHeightFt);
    });

    it('matches the in-window floor continuously at the window edge', () => {
      const beach = {
        shoaling_factors: BLACKS_BUCKETS,
        swell_window_center_deg: 282.5,
        swell_window_halfwidth_deg: 102.5,
        swell_access_factors: accessArray({ [toBin5(25)]: 0 }),
      };
      const before = transformToFaceHeightWithMetadata({
        rawHeightFt: 2,
        periodS: 14,
        swellDirectionDeg: 24.9,
        source: 'cdip_sig',
        beach,
        directionScoringEnabled: true,
      });
      const after = transformToFaceHeightWithMetadata({
        rawHeightFt: 2,
        periodS: 14,
        swellDirectionDeg: 25.1,
        source: 'cdip_sig',
        beach,
        directionScoringEnabled: true,
      });

      expect(Math.abs((before.directionFactor ?? 0) - (after.directionFactor ?? 0))).toBeLessThan(0.01);
    });

    it('degrades to a neutral factor when direction or window data is missing', () => {
      const base = {
        rawHeightFt: 2,
        periodS: 14,
        source: 'cdip_sig' as const,
        beach: { shoaling_factors: BLACKS_BUCKETS },
        directionScoringEnabled: true,
      };
      expect(transformToFaceHeightWithMetadata({ ...base, swellDirectionDeg: null }).directionFactor).toBe(1);
      expect(transformToFaceHeightWithMetadata({
        ...base,
        swellDirectionDeg: 202,
        beach: { ...base.beach, swell_window_center_deg: null, swell_window_halfwidth_deg: null },
      }).directionFactor).toBe(1);
    });
  });
