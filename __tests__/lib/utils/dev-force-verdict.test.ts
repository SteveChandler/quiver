...
function makeResult(overrides: Partial<SurfCallResult> = {}): SpotSurfReportResult {
  const report: SurfCallResult = {
    verdict: 'YES',
...
describe('applyForceVerdict', () => {
  ...
  it('synthesizes a peakTime±1h window when forcing MAYBE on a NO result (windows are null)', () => {
    // Code-review P0-1: on a flat day the upstream verdict is NO and
    // bestWindowStart/End arrive null. Forcing MAYBE for a walkthrough
    // must still produce a narrowed window, otherwise the UI renders
    // "MAYBE" with no window row and the walkthrough misrepresents the
    // real-world MAYBE case.
    const result = makeResult({
      verdict: 'NO',
