...
function registrationMetadata(row: Record<string, unknown>): Json {
  const metadata: Record<string, string | number | boolean | null> = {};
  for (const [key, value] of Object.entries(row)) {
    if (["user_id", "platform", "device_token"].includes(key)) continue;
    if (
      value === null ||
      typeof value === "string" ||
      typeof value === "number" ||
  ...
  async (request: NextRequest, { user, supabase }) => {
    ...
    if (
      ...
      return NextResponse.json(
        { success: false, error: "installation_id must be a non-empty string" },
        { status: 400 },
      );
    }
    ...
    const metadata = registrationMetadata(upsertRow);
    if (typeof installation_id === "string" && installation_id.trim()) {
      const result = await supabase.rpc("register_device_installation", {
        p_user_id: user.id,
      ...
      const result = await supabase.rpc("register_legacy_device_token", {
        p_user_id: user.id,
        p_platform: platform,
        p_device_token: device_token,
        p_metadata: metadata,
...
export const DELETE = withAuth(
  async (request: NextRequest, { user, supabase }) => {
    ...
    const scopedQuery = installation_id
      ? query.eq("installation_id", installation_id)
      : query.eq("device_token", device_token);
    const { error: deleteError } = await scopedQuery.is("retired_at", null);
