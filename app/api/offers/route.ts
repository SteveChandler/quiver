...
export const GET = withRateLimit(withAuth(async (request, { user }) => {
  ...
  const result = ownedOffersSchema.parse(await lifecycleRpc("list_owned_pro_offers", { p_user_id: user.id }));
  if (new URL(request.url).searchParams.get("include_cancellation_gifts") !== "true") {
    result.offers = result.offers.filter(offer => offer.program_id === "five_sessions_month" || offer.program_id === "return_three_months");
  }
