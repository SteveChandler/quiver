...
export const POST = withAdminAuth(async (request, { user }) => {
  const parsed = z.object({ userId: z.uuid(), program: z.enum(["five_sessions_month", "return_three_months", "manual_month"]),
  ...
  if (input.mode === "preview") return NextResponse.json({ status: "preview", program: input.program,
    months: input.program === "return_three_months" ? 3 : 1, historicalSessionsCount: input.program === "five_sessions_month", createsSubscription: false }, { headers });
